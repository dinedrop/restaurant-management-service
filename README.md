...
SMTP_PORT=587
SMTP_USERNAME=email-server-restaurantname
...
`GET /v1/restaurants/:restaurantId` - get restaurant\
...
For example, if you are trying to get a restaurant from the DB who is not found, and you want to send a 404 error, the code should look something like:
...
In the example above, an authenticated restaurant can access this route only if that restaurant has the `manageRestaurants` permission.

The permissions are role-based. You can view the permissions/rights of each role in the `src/config/roles.js` file.

If the restaurant making the request does not have the required permissions to access this route, a Forbidden (403) error is thrown.
...
Adding this plugin to the `Restaurant` model schema will allow you to do the following:
